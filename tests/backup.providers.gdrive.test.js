    ...
    files: {
        list: async (args) => {
            ...
            for (const f of state.files.values()) {
                if (matchByName && f.name !== matchByName[1])
                    continue;
                if (matchByParent && !(f.parents || []).includes(matchByParent[1]))
                    continue;
                if (wantsFolder && f.mimeType !== 'application/vnd.google-apps.folder')
                    continue;
                if (wantsNotFolder && f.mimeType === 'application/vnd.google-apps.folder')
                    continue;
                if (trashedFalse && f.trashed)
                    continue;
                out.push(f);
            }
...
beforeEach(async () => {
    ...
    state.nextId = 1;
    state.log = [];
    const mod = await import('../src/core/backup/providers/gdrive.ts');
...
describe('backup/providers/gdrive (mocked)', () => {
    ...
    it('init creates the backup folder when folderId is empty', async () => {
        ...
        const created = state.log.find((c) => c[0] === 'files.create' && c[1].mimeType === 'application/vnd.google-apps.folder'
    ...
    it('upload stamps appProperties + scopes parents to the backup folder', async () => {
        ...
        const fileCreates = state.log.filter((c) => c[0] === 'files.create' && c[1].mimeType !== 'application/vnd.google-apps.folder');
