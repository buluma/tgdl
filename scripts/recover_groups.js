...
function loadGroupsFromDownloadsTable() {
    ...
    return rows.map((r) => ({
        ...
        name: r.group_name && String(r.group_name).trim()
...
(function main() {
    ...
    if (noEvidence.length) {
        ...
        console.log('They had no media downloaded yet, or the table was wiped too. Review before enabling.');
    }
    ...
    if (!APPLY) {
        console.log('Dry-run only. Re-run with --apply to commit. Add --enable to flip enabled=true.');
        process.exit(0);
    }
    ...
    console.log('The folder layout under data/downloads/ is unchanged — new files go to the same paths.');
})();
